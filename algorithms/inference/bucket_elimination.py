...
class BEA(FactoredAlgorithm):
    ...
    elimination order.  The query and elimination variables must be disjoint.  Dynamic 
    programming here means that a new factor is computed only once in this run.  But in the 
    next BEA run, new factors are recomputed, since the possibly changed query changes the 
    elimination order.  As a result, the order of computing the factors can also be changed 
    and the factors computed in the previous run cannot be reused.
